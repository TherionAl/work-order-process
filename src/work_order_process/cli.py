...
from .mysql_storage import (  # noqa: F401 - dynamic command handlers import these CLI seams.
...
from .revenue_summary import (  # noqa: F401 - dynamic command handlers import this CLI seam.
    generate_revenue_summary,
)
from .schema_migrations import (  # noqa: F401 - dynamic command handlers import these CLI seams.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--month", type=int, default=None, help="只处理指定月份，取值 1-12；默认处理全年。"
    )
    parser.add_argument(
        "--sample-size", type=int, default=3, help="每个月抽取的工单详情数量，默认 3。"
    )
    parser.add_argument(
        "--seed", type=int, default=2025, help="月度抽样随机种子，默认 2025，保证可复现。"
    )
    parser.add_argument("--per-page", type=int, default=5000, help="搜索接口分页大小，默认 5000。")
    parser.add_argument(
        "--detail-workers", type=int, default=4, help="样本详情并发获取线程数，默认 4。"
    )
    parser.add_argument(
        "--limit-per-month",
        type=int,
        default=None,
        help="调试用：限制每个月最多获取多少条列表记录。",
    )
    parser.add_argument("--overwrite", action="store_true", help="覆盖已有月度输出文件。")
    parser.add_argument(
        "--metric-code", default=None, help="Only calculate one configured time metric."
    )
    parser.add_argument(
        "--metrics-config",
        default=str(DEFAULT_METRICS_CONFIG),
        help="Time metric config JSON path.",
    )
    parser.add_argument(
        "--calendar-path", default=str(DEFAULT_CALENDAR_PATH), help="Work calendar JSON path."
    )
    ...
    parser.add_argument(
        "--log-limit", type=int, default=20, help="mysql-sync-log: 显示最近多少条日志，默认 20。"
    )
    parser.add_argument(
        "--allow-empty",
        action="store_true",
    ...
    parser.add_argument(
        "--max-workers", type=int, default=8, help="并发导入时的 API 拉取线程数，默认 8。"
    )
    parser.add_argument(
        "--batch-size", type=int, default=100, help="并发导入时每批提交的事务大小，默认 100。"
    )
    parser.add_argument(
        "--api-rate-limit", type=int, default=10, help="并发导入时 API QPS 上限，默认 10。"
    )
    parser.add_argument(
        "--erp-file", default=None, help="import-erp: ERP 新旧合并数据 Excel 文件路径。"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--create-date", default=None, help="import-customer-account: 数据日期，如 20260710。"
    )
    parser.add_argument(
        "--sheet", default=None, help="import-customer-account: Sheet 名称（默认第一个）。"
    )
    return parser
...
def _print_erp_import_report(report: dict[str, Any]) -> None:
    ...
    if "reused_baseline_sales_platform" in report:
        table.add_row(
            "Reused baseline sales_platform", str(report["reused_baseline_sales_platform"])
        )
    ...
    if "applied_system_engineer_mapping" in report:
        table.add_row(
            "Applied system_engineer mapping", str(report["applied_system_engineer_mapping"])
        )
...
def _print_revenue_summary_report(report: dict[str, Any]) -> None:
    ...
    table.add_row(
        "Unmapped ERP platforms", ", ".join(report["unmapped_metric_platforms"]) or "None"
    )
...
def _print_year_report(report: dict[str, Any]) -> None:
    ...
    table.add_row(
        "total",
        str(report["ticket_total"]),
        str(report["detail_total"]),
        str(report["failed_total"]),
    )
    console.print(table)
