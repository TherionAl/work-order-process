...
def default_statistics_periods(
    year: int,
) -> tuple[tuple[str, str], tuple[str, str]]:
    """Return full previous-year and current-year allocation periods."""
    return (
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    default_previous_period, default_current_period = default_statistics_periods(
        args.statistics_year
    )
    previous_period = (
        args.last_year_start or default_previous_period[0],
        args.last_year_end or default_previous_period[1],
    )
    current_period = (
        args.current_year_start or default_current_period[0],
        args.current_year_end or default_current_period[1],
    )
