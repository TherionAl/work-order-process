...
- `work_order_process.migrations.v0003_auxiliary_snapshot_tables._partitions`：兼容不同 MySQL 版本返回的 `PARTITION_METHOD=RANGE` 或 `RANGE COLUMNS`，并结合规范化后的裸 `create_date` 表达式识别 DDL 契约 `RANGE COLUMNS(create_date)`，同时检查 `p_future/MAXVALUE`。
